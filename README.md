...
* **Tempo Real:** SignalR / Firebase Messaging.
...
### 1. Ficheiro .env
Crie um ficheiro `.env` na raiz do projeto (ao lado do `Program.cs`) ou na stack com as seguintes chaves:

```env
AZURE_CLIENT_ID=client_id_azure
DB_CONNECTION_KEY=Server=sasipca-db;Port=porta;Database=sasipca;User=user_seguro;Password=password_segura;SslMode=None;
JWT_KEY=chave_segura
DB_ROOT_PASSWORD=password_segura
FIREBASE_JSON_LOCAL_PATH=caminho/para/firebase.json
DB_APIUSER_PASSWORD=password_segura
DB_APIUSER_NAME=user_seguro

```

### 2. Firebase
Coloque o ficheiro de credenciais do Firebase na raíz do projeto para habilitar as notificações push para Android.

## Base de Dados
O projeto utiliza Entity Framework Core com MariaDB.
...
## Em Produção
Recomenda-se publicar a aplicação e correr como um container, utilizando o docker-compose e Dockerfile disponíveis.
